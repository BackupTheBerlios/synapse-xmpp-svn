...
namespace XMPP
{
	...
	{
	public:
		AMPRule(QString con1, QString act1, QString val1) : con(con1), act(act1), val(val1) {};
		QString con;
		QString act;
		QString val;
	};
	
	class AMPRules : public QList<AMPRule>
	{
	public:
		AMPRules();
		~AMPRules();
		QDomElement toXml(Stanza *);
	...
	public:
		...
		QStringList metas() const;
		int jidPriority() const;
		...
		void setMetas(const QStringList);
		void setJidPriority(int p);
		...
		bool addMeta(const QString &, int priority = 1);
		bool removeMeta(const QString &);
	...
	public:
		...
		void setRosterItem(const RosterItem &, bool setMeta = false);
