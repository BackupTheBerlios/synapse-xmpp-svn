...
QDomElement AMPRules::toXml(Stanza *s)
{
	QDomElement amp = s->createElement("http://jabber.org/protocol/amp","amp");

	amp.setAttribute("per-hop",(perHop_) ? "true" : "false");

	for(AMPRules::Iterator it = begin(); it != end(); ++it)
	{
		QDomElement rule = s->doc().createElement("rule");
		rule.setAttribute("condition",(*it).con);
		rule.setAttribute("action",(*it).act);
		rule.setAttribute("value",(*it).val);
		amp.appendChild(rule);
	}
	
	return amp;
}

void AMPRules::fromXml(QDomElement &e)
{
	status_ = e.attribute("status");
	from_ = e.attribute("from");
	for(QDomNode n = e.firstChild(); !n.isNull(); n = n.nextSibling())
	{
		QDomElement r = n.toElement();
		AMPRule rule = AMPRule(r.attribute("condition"), r.attribute("action"), r.attribute("value"));
		append(rule);
	}
}
...
Stanza Message::toStanza(Stream *stream) const
{
	...
	if (!d->amp_rules->isEmpty()) {
		s.appendChild(d->amp_rules->toXml(&s));
	}
...
bool Message::fromStanza(const Stanza &s, int timeZoneOffset)
{
	...
	if (nl.item(0).toElement().tagName() == "nos:x")
		return false;
	...
	for(n = 0; n < nl.count(); ++n) {
		...
		if(i.isElement()) {
			...
			else if(e.tagName() == "amp" && e.namespaceURI() == "http://jabber.org/protocol/amp") {
				d->amp_rules->fromXml(e);
			}
			else {
...
bool RosterItem::addMeta(const QString &m, int priority)
{
	v_metas += m;
	v_jidPriority = priority;
	return true;
}
...
bool RosterItem::removeMeta(const QString &m)
{
	for(QStringList::Iterator it = v_metas.begin(); it != v_metas.end(); ++it) {
		if(*it == m) {
			v_metas.remove(it);
			return true;
		}
...
void RosterItem::setMetas(const QStringList m)
{
	v_metas = m;
}

QStringList RosterItem::metas() const
{
	return v_metas;
}

void RosterItem::setJidPriority(int p)
{
	v_jidPriority = p;
}

int RosterItem::jidPriority() const
{
	return v_jidPriority;
}
