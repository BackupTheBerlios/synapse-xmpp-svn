...
void JT_AMP::set(const Jid &j)
{
	d->jid = j;

	d->iq = createIQ(doc(), "get", d->jid.domain(), id());
	QDomElement query = doc()->createElement("query");
	query.setAttribute("xmlns", "http://jabber.org/protocol/disco#info");
	query.setAttribute("node", "http://jabber.org/protocol/amp");
...
bool JT_AMP::take(const QDomElement &e)
{
	if(!iqVerify(e, d->jid, id()))
		return false;

	if(e.attribute("type") == "result") {
		bool found;
		QDomElement q = findSubTag(e, "query", &found);
		if (found && q.attribute("node") == "http://jabber.org/protocol/amp")
		{
			d->features.clear();
			for(QDomNode n = q.firstChild(); !n.isNull(); n = n.nextSibling())
			{
				...
				if(f.isNull())
					continue;
				if(f.tagName() == "feature")
					d->features << f.attribute("var");
			}
		}
		setSuccess(true);
	}
...
bool JT_AMP::test(const QString &feature)
{
