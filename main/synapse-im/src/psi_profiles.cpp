...
QDomElement UserAccount::toXml(QDomDocument &doc, const QString &tagName)
{
	...
	setBoolAttribute(a, "amp", opt_amp);
...
void UserAccount::fromXml(const QDomElement &a)
{
	...
	if (a.hasAttribute("autostatus"))
	{
		...
		readEntry(a, "autostatus-status", &opt_login_status);
		readEntry(a, "autostatus-message", &opt_login_message);
	}	
...
void UserProfile::reset()
{
	...
	prefs.color[cOnline]    = QColor("#00ab11");
	prefs.color[cListBack]  = QColor("#FFFFFF");
	prefs.color[cAway]      = QColor("#959c0b");
	prefs.color[cDND]       = QColor("#b40000");
	prefs.color[cOffline]   = QColor("#535353");
	prefs.color[cStatus]    = QColor("#636363");
	prefs.color[cProfileFore] = QColor("#000000");
	prefs.color[cProfileBack] = QColor("#fefefe");
	prefs.color[cGroupFore] = QColor("#1b1b1b");
	...
	prefs.font[fStatus] = QApplication::font().toString();
	...
	prefs.serviceRosterIconset["tlen"]	= "amirose-hantu-tlen.jisp";
...
bool UserProfile::toFile(const QString &fname)
{
...

	{
...

		{
			...
			tag.appendChild(textTag(doc, "status", prefs.font[fStatus] ));
		}
	}
	...
	if(found) {
		...
		if(found) {
			...
			if(found) {
				...
				readEntry(tag, "status", &prefs.font[fStatus]);
			}
		}
