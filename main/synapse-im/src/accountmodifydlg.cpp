...
static QString statusmap[6] = {"chat","","away","xa","dnd"};
...
void AccountModifyDlg::init() 
{
	...
	QStringList status;
	status << status2txt(STATUS_CHAT) << status2txt(STATUS_ONLINE) << status2txt(STATUS_AWAY) << status2txt(STATUS_XA) << status2txt(STATUS_DND);
	cb_login_status->insertStringList(status);
	connect(cb_login_status, SIGNAL(highlighted(int)), SLOT(selectStatus(int)));
	int n = 1;
	for(n=0;n<5;n++)
	{
		if(acc.opt_login_status.compare(statusmap[n])==0)
			break;
	}
	cb_login_status->setCurrentIndex(n);
	tb_login_message->setText(acc.opt_login_message);
...
void AccountModifyDlg::selectStatus(int x)
{
	cb_login_status->setCurrentItem(x);
}
...
void AccountModifyDlg::save()
{
	...
	acc.opt_login_message = tb_login_message->text();
