...
PsiToolBar *PsiToolBar::fromOptions(const QString &base, QMainWindow* mainWindow, PsiCon* psi, PsiActionList::ActionsType t)
{
	PsiToolBar *tb = 0;

	tb = new PsiToolBar(PsiOptions::instance()->getOption(base + ".name").toString(), mainWindow, psi);
/*	mainWindow->moveDockWindow ( tb, 
	...
	mainWindow->addToolBar((Qt::ToolBarArea)PsiOptions::instance()->getOption(base + ".dock.position").toInt(), tb);
	return tb;
}
