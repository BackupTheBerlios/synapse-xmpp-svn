...
//#endif
	...
	void actionMetaAdd(const Jid &, const QString &, int priority);
	void actionMetaRemove(const Jid &, const QString &);
...
private:
	...
	ContactViewItem *addMeta(const QString &name, ContactViewItem *group_item);
	ContactViewItem *checkMeta(const QString &name, ContactViewItem *group_item);
	ContactViewItem *ensureMeta(const QString &name, ContactViewItem *group_item);
...
class ContactViewItem : public QObject, public RichListViewItem
{
	Q_OBJECT
public:
	enum { Profile, Group, Contact, Meta };
	...
	ContactViewItem(const QString &metaName, ContactProfile *, ContactViewItem *parent);
	...
	void resetStatus();
	void resetMetaStatus();
	void resetName(bool forceNoStatusMsg = false); // use this to cancel a rename
	void resetMetaName(bool forceNoStatusMsg = false); // use this to cancel a rename
	void resetGroupName();

	void updatePosition();
	void optionsUpdate();

	UserListItem *priority();
	...
	QString giveContactJID ();
	QString giveContactStatusMessage ();
public slots:
	void resetAnim();
	void iconUpdated();
	void animateNick();
