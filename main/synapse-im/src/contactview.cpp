...
public:
	...
	Q3PtrList<ContactViewItem> metas;
	int oldstate;
	QTimer *t;
	PsiAccount *pa;
	bool v_enabled;
...
GMailNotify *ContactProfile::gMailNotify()
{
	return d->su.gMailNotify();
}
...
ContactViewItem *ContactProfile::addMeta(const QString &name, ContactViewItem *group_item)
{
	ContactViewItem *item;
	item = new ContactViewItem(name, this, group_item);
...
// check for meta contacts
ContactViewItem *ContactProfile::checkMeta(const QString &name, ContactViewItem *group_item)
{
	ContactViewItem *item = group_item;
	item = (ContactViewItem *)item->firstChild();
	for(; item; item = (ContactViewItem *)item->nextSibling()) {
		if(item->type() == ContactViewItem::Meta && item->metaName() == name)
		{
			return item;
		}
	}
...
ContactViewItem *ContactProfile::ensureMeta(const QString &name, ContactViewItem *group_item)
{
	ContactViewItem *meta_item = checkMeta(name, group_item);
	if(!meta_item)
		meta_item = addMeta(name, group_item);
...
void ContactProfile::removeContactItem(Entry *e, ContactViewItem *i)
{
	...
	ContactViewItem *item = (ContactViewItem *)static_cast<Q3ListViewItem *>(i)->parent();
	//printf("ContactProfile: removing [%s] from group [%s]\n", e->u.jid().full().latin1(), group_item->groupName().latin1());
	e->cvi.removeRef(i);
	deferredUpdateGroups();
	if((item)->type() == ContactViewItem::Group)
		checkDestroyGroup(item);
	else if((item)->type() == ContactViewItem::Meta)
	{
		ContactViewItem *g = (ContactViewItem *)static_cast<Q3ListViewItem *>(item)->parent();
		checkDestroyMeta(item);
		if((g)->type() == ContactViewItem::Group)
		checkDestroyGroup(g);
	}
}
...
void ContactProfile::addNeededContactItems(Entry *e)
{
	...
	if(u.isPrivate())
	...
	else if(u.groups().isEmpty() && u.metas().isEmpty())
		ensureContactItem(e, ensureGroup(ContactViewItem::gGeneral));
	else if(u.groups().isEmpty() && !u.metas().isEmpty()) {
		const QStringList &metas = u.metas();
		for(QStringList::ConstIterator mit = metas.begin(); mit != metas.end(); ++mit)
			ensureContactItem(e, ensureMeta(*mit, ensureGroup(ContactViewItem::gGeneral)));
	}
	else if(u.metas().isEmpty()){
		...
		for(QStringList::ConstIterator git = groups.begin(); git != groups.end(); ++git)
			for(QStringList::ConstIterator mit = metas.begin(); mit != metas.end(); ++mit)
				ensureContactItem(e, ensureMeta(*mit, ensureGroup(*git)));
	}
}
...
void ContactProfile::removeUnneededContactItems(Entry *e)
{
	...
	for(ContactViewItem *i; (i = it.current());) {
		...
		if(g->type() != ContactViewItem::Meta && !u.metas().isEmpty())
			del = true;
		...
		else if(g->type() == ContactViewItem::Meta)
		{
			if(u.metas().isEmpty())
				del = true;
		}
...
void ContactProfile::ensureVisible(Entry *e)
{
	...
	if(!i)
		return;
	ContactViewItem *parent = (ContactViewItem*)i->Q3ListViewItem::parent();
	if(parent && (parent->type() == ContactViewItem::Meta))
		d->cv->ensureItemVisible(parent);
	else
...
void ContactProfile::doContextMenu(ContactViewItem *i, const QPoint &pos)
{
	...
	if(i->type() == ContactViewItem::Profile) {
		...
		if(d->pa->serverInfoManager()->hasGoogleArchive())
		{
			...
			{
				pm.insertItem(tr("Disable archiving"),13);
			}else{
				pm.insertItem(tr("Enable archiving"),13);
			}
		}
		...
		if(!option.lockdown.roster) {
			...
			pm.insertSeparator();
			pm.insertItem(tr("Send status to group"),9);
			if(!online)
	...
	else if(i->type() == ContactViewItem::Contact || i->type() == ContactViewItem::Meta) {
		bool self = false;
		UserListItem *u;
		Entry *e = 0;
		if(i == d->self) {
			...
			if(i->type() == ContactViewItem::Meta)
			{
				u = i->priority();
				e = findEntry(u->jid());
			} else {
		...
		int base_meta = base_gc + at_gc;
		int at_bm = 0;

		Q3PopupMenu *mm = new Q3PopupMenu(&pm);
		QStringList ml;
		if(!self) {
			if(inList) {
				if(!option.lockdown.roster) {
					d->cv->qa_ren->setEnabled(online);
					d->cv->qa_ren->addTo(mm);
				}
			}
			...
			if(!isAgent) {
				...
				if(inList && !option.lockdown.roster) {
					Q3PopupMenu *bm = new Q3PopupMenu(mm);
					at_bm++;
					bm->setCheckable(true);
					bm->insertItem(tr("&None"), at_bm + base_meta);
					bm->insertSeparator();
					
					QString m;
					if(e->u.metas().isEmpty())
						bm->setItemChecked(at_bm+base_meta, true);
					else
					{
						ContactViewItem *item = (ContactViewItem*)i->Q3ListViewItem::parent();
						if(item && item->type() == ContactViewItem::Meta)
						    m = item->metaName();
					}
						
					at_bm++;
					
					Q3PtrListIterator<ContactViewItem> it(d->metas);
					for(ContactViewItem *mc; (mc = it.current()); ++it)
					{
						if(mc)
						{
						ml << (*mc).metaName();
						bm->insertItem((*mc).metaName(), at_bm+base_meta);
						if((*mc).metaName() == m)
							bm->setItemChecked(at_bm+base_meta, true);
						if(!e->u.metas().isEmpty())
							bm->setItemEnabled(at_bm+base_meta, false);
						at_bm++;
						}
					}
					
					bm->insertSeparator();
					bm->insertItem(/*IconsetFactory::iconPixmap("psi/edit/clear"),*/ tr("&Create new..."), at_bm+base_meta);
					at_bm++;
					mm->insertItem(tr("&Meta"), bm, base_meta);
				}
			}
		}

		int base_group = base_meta + at_bm;
		if(!self) {
			if(!isAgent) {
				ContactViewItem *item = (ContactViewItem*)i->Q3ListViewItem::parent();
				if (item && (item->type() != ContactViewItem::Meta))
				if(inList && !option.lockdown.roster) {
					Q3PopupMenu *gm = new Q3PopupMenu(mm);
					...
					if(e->u.groups().isEmpty())
						gm->setItemChecked(at_bm+base_meta, true);
					else
						g = groupNameCache;
					...
					for(QStringList::ConstIterator it = gl.begin(); it != gl.end(); ++it) {
						...
						if(!e->u.groups().isEmpty())
							gm->setItemEnabled(n+base_group, false);
						++n;
					}
					if(n > 0)
						gm->insertSeparator();
					...
					mm->insertItem(tr("&Group"), gm, 5);

					if(!online)
						mm->setItemEnabled(5, false);
				}
			}
			else {
				pm.insertSeparator();
		...
		if(inList && !option.lockdown.roster) {
			Q3PopupMenu *authm = new Q3PopupMenu (mm);
			...
			mm->insertItem (IconsetFactory::iconPixmap("psi/register"), tr("Authorization"), authm, 20);
			if(!online)
				mm->setItemEnabled(20, false);
		}
		...
		if (PsiOptions::instance()->getOption("options.ui.menu.contact.custom-picture").toBool()) {
			Q3PopupMenu *avpm = new Q3PopupMenu(mm);
		...
		if(PGPUtil::pgpAvailable() && PsiOptions::instance()->getOption("options.ui.menu.contact.custom-pgp-key").toBool()) {
			if(u->publicKeyID().isEmpty())
				...
				mm->insertItem(IconsetFactory::icon("psi/gpg-no").icon(), tr("Unassign Open&PGP key"), 22);
		}

		pm.insertItem(tr("Manage"), mm);

		pm.insertItem(tr("Sen&d Status"), 29);
		...
		pm.insertItem(tr("Copy &JID to clipboard"), 27);
		if (i->hasStatusMessage ())
			pm.insertItem(tr("Copy &status message to clipboard"), 28);
		...
		if(x == 0) {
			...
			else if(i->type() == ContactViewItem::Meta)
			{
				ContactViewItem *item = (ContactViewItem*) i->firstChild();
				while(item && !item->isAlerting())
				{
					item = (ContactViewItem*)item->nextSibling();
				}
				if(item)
					actionRecvEvent(item->u()->jid());
			}
		}
		else if(x == 1) {
			...
			QString cliptext = i->giveContactJID ();
			...
			QClipboard *clipboard = QApplication::clipboard();
			QString cliptext = i->giveContactStatusMessage ();
		...
		else if(x == (base_meta+1)) {
			if(!u->metas().isEmpty()) {
				ContactViewItem *item = (ContactViewItem*)i->Q3ListViewItem::parent();
				actionMetaRemove(u->jid(), item->metaName());
			}
		}
		else if(x >= (base_meta+2) && x<(base_group-1)) {
			int n = 0;
			int n2 = x - (base_meta+2);
			QString newmeta;
			...
			bool ok = false;
			int newmeta_priority = QInputDialog::getInteger(tr("Set priority for contact"), tr("Enter the priority:"), 1, 1,100,1, &ok, d->cv);
			actionMetaAdd(u->jid(), newmeta, newmeta_priority);
		}
		else if(x == (base_group-1)) {
			while(1) {
				bool ok = false;
				QString newmeta = QInputDialog::getText(tr("Create New Meta"), tr("Enter the new Meta name:"), QLineEdit::Normal, QString::null, &ok, d->cv);
				int newmeta_priority = QInputDialog::getInteger(tr("Set priority for contact"), tr("Enter the priority:"), 1, 1,100,1, &ok, d->cv);
				if(!ok)
					break;
				if(newmeta.isEmpty())
			...
			if(online) {
				...
				if(n == n2) {
					...
					if(!u->groups().isEmpty()) {
						...
						{
							for(ContactViewItem *item = (ContactViewItem*)i->firstChild(); item; item = (ContactViewItem*)item->nextSibling())
								actionGroupRemove(item->u()->jid(),gname);
						}
					}
					// add the group
					if(i->type() == ContactViewItem::Contact)
					...
					{
						for(ContactViewItem *item = (ContactViewItem*)i->firstChild(); item; item = (ContactViewItem*)item->nextSibling())
							actionGroupAdd(item->u()->jid(),newgroup);
					}
				}
			}
		}
...
void ContactProfile::scSendMessage(ContactViewItem *i)
{
	...
	if(i->type() == ContactViewItem::Meta)
		actionSendMessage(i->priority()->jid());
}
	...
	bool doToolTip(QPoint pos)
	{
		...
		if(i) {
			...
			if(i->type() == ContactViewItem::Contact)
				...
				PsiToolTip::showText(globalPos, i->priority()->makeTip(true, false), cv);
...
void RichListViewItem::setStatus(int status)
{
	v_status = status;
}
...
void RichListViewItem::scaleAvatar()
{
	...
	{
		avatar = v_avatarFactory->getAvatar(v_jid.bare());
...
void RichListViewItem::setup()
{
	...
	if (v_synapseStyle) {
		...
		scaleAvatar();
//	really good idea?
 		if((v_status == STATUS_OFFLINE) && (avatar_x != 0))
			greyscaleAvatar();
// 			avatar = avatar.fromImage(avatar.toImage(),Qt::MonoOnly);

// 		int left =  lv->itemMargin() + ((px)?(px->width() + lv->itemMargin()):0);

// 		int left = lv->itemMargin() + v_avatarSize + lv->itemMargin();
...
void RichListViewItem::setJid(const Jid& jid)
{
	v_jid = jid;
}

void RichListViewItem::setAvatarFactory(AvatarFactory *af)
{
	v_avatarFactory = af;
}
...
ContactViewItem::ContactViewItem(const QString &metaName, ContactProfile *cp, ContactViewItem *parent)
:RichListViewItem(parent)
{
	type_ = Meta;
	d = new Private(this, cp);
	d->metaName = metaName;
	setAvatarFactory(cp->psiAccount()->avatarFactory());
	d->alerting = false;
...
void ContactViewItem::cacheValues()
{
	...
	if ( u ) {
		if( !u->isAvailable() )
			...
			d->status = makeSTATUS((*u->priority()).status());
		d->isAgent = u->isTransport();
	}
}
...
const QString & ContactViewItem::metaName() const
{
	return d->metaName;
}
...
void ContactViewItem::paintCell(QPainter *p, const QColorGroup & cg, int column, int width, int alignment)
{
	if ( type_ == Contact || type_ == Meta) {
		...
		if ( d->u )
			...
			u = priority(); 
		if ( u ) {
			UserResourceList::ConstIterator it = u->priority();
			if(it != u->userResourceList().end()) {
				if(u->isSecure((*it).name())) {
...
{
	RichListViewItem::setStatus(d->status);
	if ( !d->alerting && d->u ) {
		setIcon(PsiIconset::instance()->statusPtr(d->u));
	}
	
	ContactViewItem *cvi = (ContactViewItem *) Q3ListViewItem::parent();
	if (cvi && cvi->type() == ContactViewItem::Meta)
		cvi->resetMetaStatus();
...
UserListItem *ContactViewItem::priority()
{
	ContactViewItem *child1 = static_cast<ContactViewItem*>(firstChild());
	if(child1!=0)
	{
		bool child1_is = (child1->status() != XMPP::Status::Offline) ? true : false;
		ContactViewItem *child2 = 0;
		child2 = child1;
		while((child2 = static_cast<ContactViewItem*>(child2->nextSibling())) && (child2 != 0))
		{
			bool child2_is = (child2->status() != XMPP::Status::Offline) ? true : false;
			if ((!(child2_is ^ child1_is)) && (child1->u()->jidPriority() > child2->u()->jidPriority()))
			{
				child1 = child2;
				child1_is = child2_is;
			} 
			else if(child2_is && !child1_is)
			{
				child1 = child2;
				child1_is = child2_is;
			}
...
void ContactViewItem::resetMetaStatus()
{

	RichListViewItem::setStatus(d->status);
	UserListItem *u = priority();
	if (u) setJid(u->jid());
	if ( !d->alerting && u ) {
		setIcon(PsiIconset::instance()->statusPtr(u));
	}
		...
		if (!forceNoStatusMsg && static_cast<ContactView*>(Q3ListViewItem::listView())->isShowStatusMsg()) {
			...
			if (d->status_single) {
				...
				if (option.useEmoticons)
					statusMsg = TextUtil::emoticonify(statusMsg);
...
void ContactViewItem::resetMetaName(bool forceNoStatusMsg)
{
	UserListItem *u = priority();

	if ( u ) {
//		QString s = JIDUtil::nickOrJid(d->u->name(), d->u->jid().full());
		QString s = d->metaName;
...
void ContactViewItem::setAlert(const PsiIcon *icon)
{
	...
	ContactViewItem *cvpi = (ContactViewItem*)Q3ListViewItem::parent();
	if ( cvpi && (cvpi->type() == ContactViewItem::Meta) )
		cvpi->setAlert(icon);
...
void ContactViewItem::clearAlert()
{
	if ( d->alerting ) {
		bool alert = false;
		if (type_ == ContactViewItem::Meta)
		{
			ContactViewItem *item = (ContactViewItem*)firstChild();
			for(; item; item = (ContactViewItem*)item->nextSibling())
				alert = alert || item->isAlerting();
		}
		if (!alert)
		{
			d->alerting = false;
			...
			if (type_ == ContactViewItem::Contact)
				resetStatus();
			else if (type_ == ContactViewItem::Meta)
				resetMetaStatus();
		}
	}
}
...
bool ContactViewItem::hasStatusMessage () const
{
	return true;
}

QString ContactViewItem::giveContactJID ()
{
	if(type_ == ContactViewItem::Contact)
		return d->u->jid().full();
	else if(type_ == ContactViewItem::Meta)
		return priority()->jid().full();
}

QString ContactViewItem::giveContactStatusMessage ()
{
	UserListItem *u = 0;
	if(type_ == ContactViewItem::Contact)
	{
	...
	if (u->priority() != u->userResourceList().end())
		return (*u->priority()).status().status();
	return u->lastUnavailableStatus().status();
}
