...
/*	d->cp = new ContactProfile(this, acc.name, d->psi->contactView());
	...
	connect(d->cp, SIGNAL(actionMetaAdd(const Jid &, const QString &, int)),SLOT(actionMetaAdd(const Jid &, const QString &, int)));
	connect(d->cp, SIGNAL(actionMetaRemove(const Jid &, const QString &)),SLOT(actionMetaRemove(const Jid &, const QString &)));
...
GArchive *PsiAccount::gArchive() const
{
	return d->ga;
}
...
{
//	if (d->jingleSessionManager)
//		d->jingleSessionManager->initialize();
...
bool PsiAccount::processIncomingAMP(const Message &_m)
{
	AMPRules *amp_rules = _m.ampRules();
	for(AMPRules::Iterator it = amp_rules->begin(); it!= amp_rules->end(); ++it)
	{
		QString str;
		if ((*it).con == "deliver")
		{
			if((*it).val == "direct")
				str = tr("Message delivered directly");
			else if((*it).val == "forward")
				str = tr("Message forwarded");
			else if((*it).val == "gateway")
				str = tr("Message delivered to gateway");
			else if((*it).val == "none")
				str = tr("Unable to deliver message!!");
			else if((*it).val == "stored")
				str = tr("Message is stored");
		}
		else if ((*it).con == "expire-at")
		{
			str = tr("Message delivered after specified time!");
		}
		else if ((*it).con == "match-resource")
		{
			if((*it).val == "any")
				str = tr("Message would be delivered to any resource");
			else if((*it).val == "exact")
				str = tr("Message would be delivered to exact resource");
			else if((*it).val == "other")
				str = tr("Message would be delivered to other resources");
		}
		((Message*)&_m)->setBody(str,"en");
		((Message*)&_m)->setFrom(amp_rules->from());
		if((*it).act == "alert")
		{
			((Message*)&_m)->setType("");
			return true;
		...
		if((*it).act == "error")
		{
			// error parsing AMP (msg from server)
			((Message*)&_m)->setType("error");
			return true;
		}
		if((*it).act == "notify")
		{
			((Message*)&_m)->setType("chat");
			// only notifications (msg to chat dlg if already opened)
			ChatDlg *c = findDialog<ChatDlg*>(amp_rules->from());
			if(c)
			{
			return true;
			}
...
#if defined(HAVE_JINGLE) && defined(jingle_ft)
void PsiAccount::incomingJingleFileTransfer(JingleFileTransfer* ft)
{
	if (QMessageBox::information(0, tr("Incoming file"), QString(tr("Do you want to accept %1 (%2 kb) from %3?")).arg(ft->fileName()).arg((float) ft->fileSize() / 1000).arg(ft->peer().full()), QMessageBox::Yes,QMessageBox::No | QMessageBox::Default | QMessageBox::Escape, QMessageBox::NoButton) == QMessageBox::Yes) {
		JingleFileTransferProgressDialog* d = new JingleFileTransferProgressDialog(ft);
		d->show();
		ft->accept();
	}
	else 
...
void PsiAccount::setStatus(const Status &_s,  bool withPriority, bool withPlaying)
{
