#ifndef SYNAPSE_TRANSMITTER_H

#define SYNAPSE_TRANSMITTER_H

#include "rtptransmitter.h"

class SynapseTransmitter : public RTPTransmitter
{
public:
	SynapseTransmitter();
	virtual ~SynapseTransmitter();
	...
	virtual int Poll();
	virtual int WaitForIncomingData(const RTPTime &delay, bool *dataavailable);
...
class RTPSynapseTransmissionParams : public RTPTransmissionParams
{
public:
	RTPSynapseTransmissionParams() : RTPTransmissionParams(RTPTransmitter::SynapseProto), portbase(12555) {};

	void SetPortbase(uint16_t pbase) { portbase = pbase; }
	uint16_t GetPortbase() { return portbase; }
